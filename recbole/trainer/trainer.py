...
class Trainer(AbstractTrainer):
    ...
    def evaluate(
        ...
        if load_best_model:
            checkpoint_file = model_file or self.saved_model_file
            checkpoint = torch.load(checkpoint_file, map_location=self.device, weights_only=False)
            op = checkpoint.get("other_parameter")
            self.model.load_state_dict(checkpoint["state_dict"])
            self.model.load_other_parameter(checkpoint.get("other_parameter"))
            message_output = "Loading model structure and parameters from {}".format(
                checkpoint_file
    ...
    @torch.no_grad()
    def save_SAE_activations(
        self, data, model_file=None, show_progress=True, eval_data=True, sae=True, epochs=None
    ):
        r"""Evaluate the model based on the eval data.
        ...
        times = epochs if epochs else float("inf")
        cur = 0
        for batch_idx, batched_data in enumerate(iter_data):
            if cur >= times:
                break
    ...
    def analyze_neurons_int(
        ...
        out_path = rf"./dataset/{self.dataset}/activation_counts.csv"
    ...
    def compute_neuron_stats(
        ...
        pop_activations = pop_activations.detach().float().cpu()
        unpop_activations = unpop_activations.detach().float().cpu()
